...
def build_output_language_instruction(code: str | None, *, plain_text: bool) -> str:
    ...
    if plain_text:
        return f"Write all player-visible prose strictly in {language.label} ({language.code})."
    return (
...
def get_prompt_path(code: str | None, file_name: str) -> Path | None:
    prompt_dir = _pack_dir(code) / "prompts"
    prompt_path = prompt_dir / file_name
    if prompt_path.exists():
        return prompt_path

    default_prompt_path = _pack_dir(DEFAULT_LANGUAGE_CODE) / "prompts" / file_name
